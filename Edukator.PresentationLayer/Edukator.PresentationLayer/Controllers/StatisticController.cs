...
namespace Edukator.PresentationLayer.Controllers
{
    public class StatisticController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            ViewBag.getDenemeCoursePrice = context.Courses.Where(x => x.Title == "deneme").Select(y => y.Price).FirstOrDefault();

            ViewBag.avgReview=context.Courses.Average(x => x.Review);
            ViewBag.getOver30Price = context.Courses.Where(x => x.Price > 30).Count();
            ViewBag.getLastCoursePrice = context.Courses.Where(x => x.CourseID == (context.Courses.Max(z=>z.CourseID))).Select(y => y.Price).FirstOrDefault();
            ...
            ViewBag.avgPriceForSoftwareCourse = context.Courses.Where(x => x.CategoryID == (context.Categories.Where(y => y.CategoryName == "Yazılım").Select(z => z.CategoryID).FirstOrDefault())).Average(h => h.Price);
            ViewBag.last3CourseTotalPrice=context.Courses.OrderByDescending(x=>x.CourseID).Take(3).Sum(y => y.Price);
            ViewBag.getCourseCategorySoftwareAndReviewMore20Score = context.Courses.Where(x => x.CategoryID == (context.Categories.Where(y => y.CategoryName == "Yazılım").Select(z => z.CategoryID).FirstOrDefault()) && x.Review >= 20).Select(w => w.Title).FirstOrDefault(); 
